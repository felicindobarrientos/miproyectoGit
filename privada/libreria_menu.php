...
if (isset($_SESSION["sesion_id_rol"])) {
	...
	$sql = $db->Prepare("SELECT ac.*, op.id_opcion, op.orden, op.contenido, gr.id_grupo, gr.grupo, op.opcion
			FROM _accesos ac, _opciones op, _grupos gr
...
if ($nick != "") {
	echo"<div id='header'>
		...
		if ($cuerp == false) {
			echo"<a onclick='location.href=\"../claves/\"'><input type='button'name='accion' value='Cerrar Sesion' style='cursor:pointer;border-radius:10px;font-weight:bold;height: 25px;' class='boton_cerrar'></a>";
		}else{
			echo"<a onclick='location.href=\"claves/\"'><input type='button'name='accion' value='Cerrar Sesion'style='cursor:pointer;border-radius:10px;font-weight: bold;height: 25px;' class='boton_cerrar'></a>";
	}
	echo"</ul>
